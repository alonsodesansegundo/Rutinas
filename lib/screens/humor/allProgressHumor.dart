...
///Pantalla que le permite al terapeuta ver las partidas de todos los jugadores en el juego Humor e incluso buscar partidas
///por nombre de jugador y/o nivel
class AllProgressHumor extends StatefulWidget {
  @override
  AllProgressHumorState createState() => AllProgressHumorState();
}

/// Estado asociado a la pantalla [AllProgressHumor] que gestiona la lógica
/// y la interfaz de usuario de la pantalla
class AllProgressHumorState extends State<AllProgressHumor> {
            ...
            children: [
              ...
              FutureBuilder<void>(
                future: getAllPartidasView(paginaActual, partidasPagina,
                    txtBuscar, selectedNivel, 'Ironias'),
            ...
            ElevatedButton(
              onPressed: () {
                for (int i = 0; i < partidasToRemove.length; i++)
                  _removePartidaHumor(partidasToRemove[i]);
                flagCheck = [];
                partidasToRemove = [];
                setState(() {
                  _loadProgresos();
  ...
  Future<void> _loadProgresos() async {
    PartidasPaginacion aux = await getAllPartidasView(
        paginaActual, partidasPagina, txtBuscar, selectedNivel, 'Ironias');
  ...
  ///Método que nos permite eliminar una partida del juego Humor a partir de su identificador
  ///<br><b>Parámetros</b><br>
  ///[partidaId] Identificador de la partida que queremos eliminar
  void _removePartidaHumor(int partidaId) {
